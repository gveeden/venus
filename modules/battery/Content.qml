...
Rectangle {
    ...
    // radius is handled by PanelWindow
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Text {
                    text: {
                        ...
                        if (UPower.onBattery) return "󱊣 Discharging"
                        return "󱊦 Charging"
                    }
            ...
            Rectangle {
                ...
                Rectangle {
                    ...
                    color: {
                        if (!UPower.displayDevice) return Appearance.colors.primary
                        const percent = UPower.displayDevice.percentage * 100
                        if (percent < 20) return Appearance.colors.secondary
    ...
    function getBatteryIcon(level) {
        ...
        if (level >= 0.7) return "󰂀"
        if (level >= 0.5) return "󰁾"
        if (level >= 0.3) return "󰁼"
        if (level >= 0.1) return "󰁺"
        return "󰂎"
    }
